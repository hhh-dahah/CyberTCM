...
if st.session_state["current_page"] == "main":
    ...
    with header_cols[1]:
        ...
        try:
            ...
            st.markdown("<!-- 动图加载失败 -->", unsafe_allow_html=True)
...
with col1:
    if st.button("🧬 体质问卷", use_container_width=True,
                 type="primary" if st.session_state["active_tab"] == 0 and st.session_state.get("current_page") == "main" else "secondary"):
        st.session_state["active_tab"] = 0
        st.session_state["current_page"] = "main"
        st.rerun()
with col2:
    if st.button("📸 舌象解码", use_container_width=True,
                 type="primary" if st.session_state["active_tab"] == 1 and st.session_state.get("current_page") == "main" else "secondary"):
        st.session_state["active_tab"] = 1
        st.session_state["current_page"] = "main"
        st.rerun()
with col3:
    if st.button("🔮 体质报告", use_container_width=True,
                 type="primary" if st.session_state["active_tab"] == 2 and st.session_state.get("current_page") == "main" else "secondary"):
        st.session_state["active_tab"] = 2
        st.session_state["current_page"] = "main"
        st.rerun()
with col4:
    if st.button("📊 数据管理", use_container_width=True,
                 type="primary" if st.session_state["active_tab"] == 3 and st.session_state.get("current_page") == "main" else "secondary"):
...
with col_center:
    is_join_page = st.session_state.get("current_page") == "join_us"
    if st.button("🎉 加入我们", use_container_width=True, type="primary" if is_join_page else "secondary"):
        st.session_state["current_page"] = "join_us"
        st.rerun()
...
# --- 加入我们页面 ---
if st.session_state.get("current_page") == "join_us":
    # 返回按钮
...
# --- 主页面内容 ---
elif st.session_state.get("current_page") == "main":
